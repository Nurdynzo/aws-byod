...
export default function Home() {
  ...
  const registerDomain = async () => {
    ...
    try {
      const response = await useRequest.post(`/register-domain`,
  ...
  const validateDomain = async () => {
    ...
    try {
      const response = await useRequest.post(`/validate`,
        { domain_name: domain.domainName },
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  const checkDnsStatus = async () => {
    ...
    try {
      const response = await useRequest.post(`/check-status`,
        { domain_name: domain.domainName },
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  const completeSetup = async () => {
    ...
    try {
      const response = await useRequest.post(`/update-alb`,
        { domain_name: domain.domainName },
        {
          headers: {
            'Content-Type': 'application/json',
        ...
        return (
          <Box mt={3}>
            ...
            <Box mt={2}>
              ...
              >
                {domain.loading ? <CircularProgress size={24} /> : 'Register Domain'}
              </Button>
            </Box>
            {domain.message && (
            ...
            <Paper sx={{ p: 2, mt: 2 }}>
              ...
              <Box sx={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>Record Type</th>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td style={{ border: '1px solid #ddd', padding: '8px' }}>{domain.domainName}</td>
                      <td style={{ border: '1px solid #ddd', padding: '8px' }}>{domain.alb_dns}</td>
                    </tr>
                    <tr>
                      <td style={{ border: '1px solid #ddd', padding: '8px' }}>{domain.CNAME}</td>
                      <td style={{ border: '1px solid #ddd', padding: '8px' }}>{domain.name}</td>
                      <td style={{ border: '1px solid #ddd', padding: '8px' }}>{domain.value}</td>
                    </tr>
                  </tbody>
                </table>
              </Box>
              ...
              <Box mt={2}>
                <Button
                  ...
                  disabled={domain.loading}
                >
                  {domain.loading ? <CircularProgress size={24} /> : 'Check DNS Status'}
                </Button>
              </Box>
            </Paper>
          ...
          <Box mt={3}>
            ...
            <Paper sx={{ p: 2, mt: 2 }}>
              <Typography variant="body1" gutterBottom>
                All DNS records have been validated. You can now complete the setup process.
              </Typography>
              ...
              <Box mt={2}>
                ...
                >
                  {domain.loading ? <CircularProgress size={24} /> : 
                   domain.status === 'completed' ? 'Setup Complete' : 'Complete Setup'}
                </Button>
              </Box>
              
              {domain.status === 'completed' && (
                <Alert severity="success" sx={{ mt: 2 }}>
                  Your domain has been successfully configured! You can now access your application at {domain.domainName}.
                </Alert>
              )}
            </Paper>
